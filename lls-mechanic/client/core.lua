                    ...
                    if (not tempPos.whitelistJobName or jobName == tempPos.whitelistJobName) then
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if (not IsHudHidden()) then
                                DrawHelpText(Config.Keys.action.name .. ' pour customiser le véhicule.', true)
                            end
        ...
        if (data.what == 'menu') then
            ...
            elseif (data.user == 'enter') then
                ...
                if ((not isOpenByAdmin) and menuOption and menuOption.price) then
                    ESX.TriggerServerCallback('GetSocietyMoney', function(acount)
                        acountt = acount
                        ...
                        elseif (menuOption.price > 0) then
                            ...
                            if acountt.acount >= tempPrice then
                                if (getPlayerCashMoney() >= 0) then
                                    canBuyMod = true
                                    if (tempPrice > 0) then
                                        TriggerServerEvent('lls-mechanic:removeCash', tempPrice)
                                    end
                                end
                            end
                        end
                ...
                if (data.color ~= nil) then
                    ...
                    if (not isOpenByAdmin) then
                        ESX.TriggerServerCallback('GetSocietyMoney', function(account)
                            accountt = account
                        

                            if accountt.acount >= tempPrice then
                                if (getPlayerCashMoney() >= 0) then
                                    if (tempPrice > 0) then 
                                        TriggerServerEvent('lls-mechanic:removeCash', tempPrice)
                                    end
